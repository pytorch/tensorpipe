...
// one component for each other device that is traversed.

const std::regex pciPathRegex(
    "/sys/devices/pci[0-9a-f]+:[0-9a-f]{2}(/[0-9a-f]+:[0-9a-f]{2}:[0-9a-f]{2}\\.[0-9a-f])+");

std::string getPciPathForIbvNic(const std::string& name) {
  std::array<char, PATH_MAX> pciPath;
  char* rv = ::realpath(
      ("/sys/class/infiniband/" + name + "/device").c_str(), pciPath.data());
  TP_THROW_SYSTEM_IF(rv == nullptr, errno);
  TP_DCHECK(rv == pciPath.data());

  std::string res(pciPath.data());
  TP_DCHECK(std::regex_match(res, pciPathRegex));
  return res;
}

std::string getPciPathForGpu(int deviceIndex) {
  ...
  TP_CUDA_CHECK(cudaDeviceGetPCIBusId(
      pciDeviceId.data(), pciDeviceId.size(), deviceIndex));
