...
std::tuple<Error, InterfaceAddresses> createInterfaceAddresses() {
  ...
  if (rv < 0) {
    return std::make_tuple(TP_CREATE_ERROR(SystemError, "getifaddrs", errno),
            InterfaceAddresses());
  }
...
std::tuple<Error, std::string> getHostname() {
  ...
  if (rv < 0) {
    return std::make_tuple(TP_CREATE_ERROR(SystemError, "gethostname", errno), std::string());
  }
...
std::tuple<Error, AddressInfo> createAddressInfo(std::string host) {
  ...
  if (rv != 0) {
    return std::make_tuple(TP_CREATE_ERROR(GetaddrinfoError, rv), AddressInfo());
  }
...
std::tuple<Error, std::string> Context::Impl::lookupAddrForIface(
  ...
  for (ifa = addresses.get(); ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    switch (ifa->ifa_addr->sa_family) {
      case AF_INET:
        return std::make_tuple(Error::kSuccess,
