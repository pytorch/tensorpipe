...
inline int xpuDeviceForPointer(const void* ptr) {
  ...
  for (int i = 0; i < devices.size(); i++) {
    ...
    // Check if the pointer belongs to this context
    auto type = sycl::get_pointer_type(ptr, ctx);

    if (type != sycl::usm::alloc::unknown) {
      // The pointer belongs to this device
      return i;
    }
  }

  // Pointer is not a USM pointer owned by any XPU device
  return -1;
}
