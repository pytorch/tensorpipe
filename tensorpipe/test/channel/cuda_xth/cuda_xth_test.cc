...
class CudaXthChannelTestSuite : public CudaChannelTestSuite {};

using namespace tensorpipe;
using namespace tensorpipe::channel;

class SendAcrossDevicesP2PDisabledTest
 ...
 private:
  void disablePeerAccess(int from, int to) {
    int canAccessPeer;
    TP_CUDA_CHECK(cudaDeviceCanAccessPeer(&canAccessPeer, from, to));
    if (canAccessPeer) {
      TP_CUDA_CHECK(cudaSetDevice(from));
      TP_CUDA_CHECK(cudaDeviceDisablePeerAccess(to));
    }
  }
  ...
  void server(std::shared_ptr<transport::Connection> conn) override {
    ...
    disablePeerAccess(0, 1);
    disablePeerAccess(1, 0);
...
CHANNEL_TEST(CudaXthChannelTestSuite, SendAcrossDevicesP2PDisabled);
